...
class GMViT(nn.Module):
    ...
    def forward(self, img):
        ...
        for xx, pos in zip(X, postion):
            ...
            cls_tokens = self.cls_token_2.expand(group_input_tokens.size(0), -1, -1)  # [B, 1, transdim]
            cls_pos = self.cls_pos_2.expand(group_input_tokens.size(0), -1, -1)  # [B, 1, transdim]

            pos = self.pos_embed_2(pos)  # [B, G, transdim]

            y = torch.cat((cls_tokens, group_input_tokens), dim=1)  # [B, G+1, transdim]
            pos = torch.cat((cls_pos, pos), dim=1)  # [B, G+1, transdim]

            y = self.blocks_2(y, pos)
            y = self.norm(y)

            concat_f = torch.cat([y[:, 0], y[:, 1:].max(1)[0]], dim=-1) # [B, transdim*2]
            feature_bank.append(concat_f.squeeze())

        ret = self.cls_head_finetune(torch.stack(feature_bank, 0))
        return ret