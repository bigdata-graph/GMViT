...
import argparse
from GMViT_mini import GMViT_mini, Student_CNN
...
def test(args, model, io):
    ...
    model.load_state_dict(torch.load('models/KD_GMViT_mini/models/model.t7'))
    model = model.eval()
    test_acc = 0.0
    count = 0.0
    test_true = []
...
if __name__ == "__main__":
    ...
    Student_model = GMViT_mini(model=student_cnn, args=args)
    if not args.eval:
